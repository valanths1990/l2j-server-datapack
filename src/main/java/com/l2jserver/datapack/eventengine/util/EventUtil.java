...
import com.l2jserver.datapack.eventengine.model.packet.CreatureSayPacket;
import com.l2jserver.datapack.eventengine.model.packet.EventMatchMessagePacket;
import com.l2jserver.datapack.eventengine.model.packet.ShowScreenMessagePacket;
		...
		switch (type)
		{
			...
			case ALL_NEAR_PLAYERS_REGISTERED:
				for (L2Npc npc : npcs)
				{
					...
					{
						if (EventEngineManager.getInstance().isRegistered(CacheManager.getInstance().getPlayer(player.getObjectId())))
						{
							list.add(player);
						}
					}
