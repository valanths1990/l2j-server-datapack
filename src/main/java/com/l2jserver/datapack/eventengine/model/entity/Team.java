...
public class Team implements IParticipant {
	...
	public Team(String teamName, TeamType teamColor) {
		_teamName = teamName;
		_teamType = teamColor;
	}
	 ...
	 */
	@Override public int getPoints(ScoreType type) {
	 ...
	 */
	@Override public void increasePoints(ScoreType type, int points) {
		if (!_points.containsKey(type))
			_points.put(type, 0);
		_points.put(type, getPoints(type) + points);
	}

	@Override public void giveItems(Collection<EItemHolder> items) {
	...
	@Override public void sendPacket(IGamePacket packet) {
		for (Player player : _players) {
