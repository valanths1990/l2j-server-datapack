...
import com.l2jserver.datapack.eventengine.enums.InventoryItemType;
...
import com.l2jserver.datapack.eventengine.managers.ItemInstanceManager;
...
import com.l2jserver.datapack.eventengine.model.instance.ItemInstance;
...
public class Player extends Playable implements IParticipant
{
	...
	private L2PcInstance cachedInstance;

	public Player(int objectId) {
		super(objectId);
		cachedInstance = L2World.getInstance().getPlayer(objectId);
	}
	...
	public L2PcInstance getPcInstance() {
		L2PcInstance p = L2World.getInstance().getPlayer(getObjectId()) ;
		return p == null ? cachedInstance : p;
	}
	...
	public String getTitle() {
		return getPcInstance().getAppearance().getVisibleTitle();
	}
	...
	public void teleportTo(LocationHolder location,boolean randomOffset) {
		getPcInstance().teleToLocation(location.getLocation(),randomOffset);
	}
	...
	public void addToEvent(Team team) {
		...
		mEventPlayerStatus._oriTitle = getPcInstance().getAppearance().getVisibleTitle();
	...
	public void removeFromEvent() {
		...
		Objects.requireNonNull(InstanceManager.getInstance().getPlayerWorld(getPcInstance())).removeAllowed(getPcInstance().getObjectId());
	...
	public void setTeam(Team team) {
		...
		getPcInstance().getAppearance().setVisibleTitle(team.getName());
...
//		L2ItemInstance l2Item = ItemInstanceManager.getInstance().getL2ItemInstance(item);
		if (item != null){
			getPcInstance().useEquippableItem(item.getObjectId(), true);
		}
	}
	...
	public void unequipItem(InventoryItemType type) {
		...
		if (l2Item != null) getPcInstance().useEquippableItem(l2Item.getObjectId(), true);
	}
