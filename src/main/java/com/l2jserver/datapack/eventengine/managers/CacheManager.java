...
public class CacheManager {
	...
	private final Map<Integer, ScheduledFuture<?>> onHold = new ConcurrentHashMap<>();
	...
	private Summon getSummon(L2Summon l2Summon, boolean initialize) {
		...
		if (summon == null && initialize)
			summon = addSummon(l2Summon);
		return summon;
	}

	public Playable getPlayable(L2Playable l2Playable, boolean initialize) {
		if (l2Playable instanceof L2PcInstance)
			return getPlayer((L2PcInstance) l2Playable, initialize);
	...
	public Character getCharacter(L2Character l2Character, boolean initialize) {
		if (l2Character instanceof L2Playable)
			return getPlayable((L2Playable) l2Character, initialize);
	...
	private Character getOtherCharacter(L2Character l2Character, boolean initialize) {
		...
		if (character == null && initialize)
			character = addOtherCharacter(l2Character);
		return character;
	}

	private Player addPlayer(L2PcInstance l2PcInstance) {
		if (onHold.containsKey(l2PcInstance.getObjectId())) {
			onHold.remove(l2PcInstance.getObjectId()).cancel(true);
		}
	...
	public void removePlayer(int objectId) {
		if (EventEngineManager.getInstance().getEventEngineState() == EventEngineState.RUNNING_EVENT && EventEngineManager.getInstance().getCurrentEvent().getPlayerEventManager().contains(objectId)) {
			ScheduledFuture<?> tmp = ThreadPoolManager.getInstance().scheduleGeneral(() -> mPlayers.remove(objectId), EventEngineManager.getInstance().getTime(), TimeUnit.SECONDS);
			onHold.put(objectId, tmp);
		} else {
