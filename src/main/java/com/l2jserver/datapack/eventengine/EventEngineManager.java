...
import com.l2jserver.datapack.eventengine.dispatcher.events.OnLogInEvent;
import com.l2jserver.datapack.eventengine.dispatcher.events.OnLogOutEvent;
...
public class EventEngineManager {
	...
	private void load() {
		try {
			...
			EventEngineAdapter.class.getDeclaredConstructor().newInstance();
			...
			// Load Npc Manager
			//			NpcManager.class.getDeclaredConstructor().newInstance();
			//						NpcData.getInstance().parseDatapackDirectory("data/eventengine/xml", true);
			...
			BypassHandler.getInstance().registerHandler(new EventEngineHandler());
			LOGGER.info(EventEngineManager.class.getSimpleName() + "Handler loaded.");
	 ...
	 *
	 * @return <li>True No registered players.</li>
	 ...
	 *
	 * @param player
	 * @return <li>True It is registered.</li>
	 ...
	 *
	 * @param player
	 * @return <li>True If the registration is successful.</li>
	 ...
	 *
	 * @param player
	 * @return <li>True If the player was registered.</li>
	...
	private final Set<Player> _playersDisconnected = ConcurrentHashMap.newKeySet();
	...
	public void returnPlayerDisconnected(Player player) {
		_playersDisconnected.stream().filter(p -> player.getObjectId() == p.getObjectId()).findAny().ifPresent(p -> p.teleportTo(p.getTeam().getRndSpawn(), true));
		_playersDisconnected.remove(player);
	}
