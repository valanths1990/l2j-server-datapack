...
public class HuntingGround extends BaseEvent<HuntingGroundConfig> {
    ...
    private final Set<L2ItemInstance> eventItems = new HashSet<>();
    ...
    protected void onUnequippedItem(OnUnequipItem event) {
        if (eventItems.contains(event.getItem()) ||
                event.getItem().getItemType() == EtcItemType.ARROW ||
                event.getItem().getItemType() == WeaponType.BOW) {
    ...
    @Override
    public void onPlayableHit(OnPlayableHitEvent event) {
    ...
    protected void onEventEnd() {
        // RewardManager
        eventItems.stream()
                .filter(i -> L2World.getInstance().getPlayer(i.getOwnerId()) != null)
                .forEach(i -> L2World.getInstance().getPlayer(i.getOwnerId())
                        .destroyItem("Event", i, null, false));
