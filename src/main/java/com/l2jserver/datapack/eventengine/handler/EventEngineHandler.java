...
public class EventEngineHandler implements IBypassHandler {
	...
	@Override public boolean useBypass(String command, L2PcInstance activeChar, L2Character bypassOrigin) {
		...
		String currentPage = action[0].split(";")[1];
		if (action.length == 1) {
			openHomepage(activeChar, command);
			return true;
		}
	...
	private void openHomepage(L2PcInstance activeChar, String command) {
		...
		html = html.replace("%time%",String.valueOf(duration.toMinutesPart()+":"+duration.toSecondsPart()));
		Player player = CacheManager.getInstance().getPlayer(activeChar.getObjectId());
