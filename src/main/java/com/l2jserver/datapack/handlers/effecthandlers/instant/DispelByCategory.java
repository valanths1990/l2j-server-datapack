...
import com.l2jserver.datapack.custom.BuffCancellation;
...
public final class DispelByCategory extends AbstractEffect {
	...
	@Override public L2EffectType getEffectType() {
	...
	@Override public void onStart(BuffInfo info) {
		...
		if (!canceled.isEmpty() && !info.getEffected().getActingPlayer().isInOlympiadMode()) {
			ThreadPoolManager.getInstance().scheduleGeneral(new BuffCancellation(info.getEffector().getActingPlayer(), info.getEffected().getActingPlayer(), canceled, true), Configuration.customs().getBuffCancellationTime());
		}
