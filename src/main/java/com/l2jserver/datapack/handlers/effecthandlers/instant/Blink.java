...
import com.l2jserver.gameserver.model.events.impl.character.player.OnPlayerMoveRequest;
...
public final class Blink extends AbstractEffect {
    ...
    public void onStart(BuffInfo info) {
        ...
        if (effected instanceof L2PcInstance) {
            TerminateReturn terminate = EventDispatcher.getInstance().notifyEvent(new OnPlayerMoveRequest((L2PcInstance) effected, destination), effected, TerminateReturn.class);
            if (terminate != null && terminate.abort()) {
                effected.sendMessage("You cannot use " + info.getSkill().getName() + " on this position.");
                return;
            }
        }
