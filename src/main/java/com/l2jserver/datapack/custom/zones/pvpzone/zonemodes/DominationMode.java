...
public class DominationMode extends ZoneMode {
    ...
    public void onZoneModeStart() {
        registerConsumer(EventType.ON_PLAYER_PVP_KILL);
        registerConsumer(EventType.ON_CREATURE_ZONE_EXIT);
        registerConsumer(EventType.ON_CREATURE_ZONE_ENTER);
        registerConsumer(EventType.ON_PLAYER_TOWER_CAPTURE);

        towers = currentZone.getTowerSpawns().stream().map(this::createNewTower).collect(Collectors.toList());
    ...
    @Override
    protected TerminateReturn receivedEvent(IBaseEvent event) {
