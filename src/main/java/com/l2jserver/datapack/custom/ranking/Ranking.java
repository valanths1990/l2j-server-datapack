...
public class Ranking implements IBypassHandler {
    ...
    public Ranking() {
        ...
        Containers.Global().addListener(new ConsumerEventListener(Containers.Players(), EventType.ON_PLAYER_PVP_KILL, this::receivedEvents, this));
        Containers.Global().addListener(new ConsumerEventListener(Containers.Players(), EventType.ON_PLAYER_ASSIST_KILL, this::receivedEvents, this));
        Containers.Global().addListener(new ConsumerEventListener(Containers.Players(), EventType.ON_PLAYER_PK_KILL, this::receivedEvents, this));
        Containers.Global().addListener(new ConsumerEventListener(Containers.Players(), EventType.ON_CLAN_REPUTATION_CHANGED, this::receivedEvents, this));
        Containers.Global().addListener(new ConsumerEventListener(Containers.Players(), EventType.ON_PLAYER_DELETE, this::receivedEvents, this));
    ...
    private void refreshHtmlTables() {
        rankingMap.forEach((key, list) -> {
            ...
            List<RankingInfo> top10Rankings = list.stream().sorted(Comparator.comparingInt(RankingInfo::getCount).reversed()).limit(10).collect(Collectors.toList());
