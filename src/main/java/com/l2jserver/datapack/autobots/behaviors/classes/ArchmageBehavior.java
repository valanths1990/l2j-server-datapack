...
import com.l2jserver.datapack.autobots.behaviors.preferences.skills.ArchmageSkillPreferences;
...
public class ArchmageBehavior extends CombatBehavior {
    public ArchmageBehavior(Autobot player, CombatPreferences combatPreferences) {
        super(player, combatPreferences);
        this.skillPreferences = new ArchmageSkillPreferences(true);
    }
    ...
    protected List<BotSkill> getSelfSupportSkills() {
        return List.of(new BotSkill(arcanePower, (player, skill, target) -> player.getCombatBehavior().getSkillPreferences().togglableSkills.containsKey(arcanePower) && !player.getEffectList().isAffectedBySkill(arcanePower)));
    }
